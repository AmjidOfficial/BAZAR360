...
export default function DealerStorefrontView({
  ...
  const socialMedia = dealer.socials || {
    facebook: dealer.socials?.facebook || '',
    instagram: dealer.socials?.instagram || '',
    whatsapp: dealer.whatsapp || '',
    tiktok: dealer.socials?.tiktok || '',
    website: dealer.socials?.website || '',
  };
  ...
  // Enable publisher for all logged in users, but restrict direct approval to owners/admins
  const showPublisher = !!currentUser;
  ...
  const handlePublishSubmit = (e: React.FormEvent) => {
    ...
    const newPost = {
      ...
      status: hasOfficialRights ? ('approved' as const) : ('pending_approval' as const),
  ...
  const dealerSchema = useMemo(() => {
    return {
      '@context': 'https://schema.org',
      '@type': 'AutoDealer',
      name: dealer.name,
      description: dealer.description,
      image: dealer.coverImage || dealer.avatarUrl || dealer.logo,
      address: {
        '@type': 'PostalAddress',
        addressLocality: dealer.location || 'Peshawar',
        addressCountry: 'PK'
      },
  ...
  return (
    <div className={`space-y-6 pb-28 relative min-h-screen bg-[var(--color-bg-primary)] transition-colors duration-500`}>
      <Helmet>
        <title>{`${dealer.name} | Verified Showroom & Inventory - Bazar360`}</title>
        <meta name="description" content={dealer.description || `Explore certified vehicle listings, showroom walkarounds, and inventory from ${dealer.name} located in ${dealer.location}.`} />
        <meta property="og:title" content={`${dealer.name} - Showroom HQ`} />
        <meta property="og:description" content={`Browse authentic car inventory and connect directly with ${dealer.name} via Bazar360.`} />
        <meta property="og:image" content={dealer.coverImage || dealer.avatarUrl || dealer.logo} />
        <meta property="og:url" content={window.location.href} />
        <meta property="og:type" content="profile" />
        <script type="application/ld+json">
          {JSON.stringify(dealerSchema)}
        </script>
      </Helmet>
      
      {/* Floating Modern Toast Notification */}
      {toastMessage && (
        <div className="fixed bottom-24 right-5 bg-[var(--color-accent-main)] border border-white/20 px-5 py-3 rounded-2xl text-xs font-mono font-bold uppercase tracking-wider text-white shadow-2xl z-55 flex items-center gap-2 animate-bounce">
      ...
      <section className={`bg-white border border-[var(--color-border-main)] rounded-3xl overflow-hidden relative shadow-[var(--shadow-bento)] select-none`}>
        ...
        <div className="h-44 md:h-64 relative overflow-hidden bg-slate-900">
          <img 
            ...
            alt={dealer.name}
            className="w-full h-full object-cover opacity-85 object-center transition-all duration-[6000ms] hover:scale-105"
            referrerPolicy="no-referrer"
          />
          {/* Neon orange/amber luxury geometric lighting accent overlay lines */}
          <div className="absolute top-0 left-0 right-0 h-[4px] bg-[var(--color-accent-main)]"></div>
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
          
          {/* Top sharing CTAs in the banner */}
          <div className="absolute top-4 right-4 flex items-center gap-2 z-10">
            <button
              onClick={() => onOpenQrModal && onOpenQrModal(dealer)}
              className="px-3.5 py-2 bg-white/90 hover:bg-white text-[var(--color-text-main)] rounded-xl text-[10px] tracking-wider uppercase font-bold border border-black/5 hover:border-black/10 flex items-center gap-1.5 transition-all shadow-sm cursor-pointer"
              title="Showroom QR Code"
            >
              <QrCode size={12} className="text-[var(--color-accent-main)] animate-pulse" /> QR Code
            </button>
            <button
            ...
            >
              <Share2 size={12} className="text-[var(--color-accent-main)]" /> Share profile
            </button>
          </div>
        ...
        <div className="px-6 pb-6 relative -mt-10 flex flex-col md:flex-row items-center md:items-end gap-5">
          ...
          <div className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-white overflow-hidden bg-white flex items-center justify-center shadow-lg relative z-10 shrink-0">
            {(dealer.logo || dealer.avatarUrl) ? (
              <img
                src={dealer.logo || dealer.avatarUrl}
                alt={dealer.name}
                className="w-full h-full object-contain p-2"
                referrerPolicy="no-referrer"
              />
            ) : (
              <span className="font-sans font-black text-4xl text-[var(--color-text-main)]">
                {dealer.avatarLetter}
              </span>
            )}
          </div>
          ...
          <div className="flex-grow text-center md:text-left space-y-2 mt-4 md:mt-0">
            <div className="flex flex-col md:flex-row items-center md:items-baseline gap-2.5 justify-center md:justify-start">
              <h2 className="font-sans font-black text-2xl md:text-3xl text-[var(--color-text-header)] uppercase tracking-tight">
                {dealer.name}
              </h2>
              {/* Verification flag */}
              <span className="px-2.5 py-1 rounded-xl bg-[var(--color-accent-main)]/10 text-[var(--color-accent-main)] text-[9px] uppercase tracking-widest font-mono font-bold border border-[var(--color-accent-main)]/20 flex items-center gap-1">
                <ShieldCheck size={11} className="text-[var(--color-accent-main)] animate-pulse" /> Certified Partner
              </span>
            </div>
            
            <div className="text-[var(--color-text-muted)] text-sm font-sans max-w-xl leading-relaxed font-medium">
              {dealer.subtitle}
            </div>

            {/* Strict Socials Hiding Rule: website is completely deleted/unrendered if missing. No blank placeholders or tags. */}
            {(socialMedia.facebook || socialMedia.instagram || socialMedia.whatsapp || socialMedia.tiktok || socialMedia.website) && (
              <div className="flex flex-wrap gap-2 pt-1 justify-center md:justify-start items-center">
                <span className="text-[10px] font-mono uppercase tracking-wider text-[var(--color-text-muted)] mr-1.5">Connect:</span>
                {socialMedia.website && (
                  <a
                    ...
                    title="Website"
                    className="w-8 h-8 rounded-lg bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] hover:bg-[var(--color-accent-main)]/10 text-[var(--color-text-main)] hover:text-[var(--color-accent-main)] flex items-center justify-center transition-all cursor-pointer"
                  >
                    <Globe size={14} />
                  </a>
                )}
                {socialMedia.whatsapp && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    title="WhatsApp"
                    className="w-8 h-8 rounded-lg bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] hover:bg-[var(--color-accent-main)]/10 text-[var(--color-text-main)] hover:text-[#25D366] flex items-center justify-center transition-all cursor-pointer"
                  >
                ...
                {socialMedia.facebook && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    title="Facebook"
                    className="w-8 h-8 rounded-lg bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] hover:bg-[var(--color-accent-main)]/10 text-[var(--color-text-main)] hover:text-[#1877F2] flex items-center justify-center transition-all cursor-pointer"
                  >
                    <Facebook size={14} />
                  </a>
                )}
                {socialMedia.instagram && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    title="Instagram"
                    className="w-8 h-8 rounded-lg bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] hover:bg-[var(--color-accent-main)]/10 text-[var(--color-text-main)] hover:text-[#E4405F] flex items-center justify-center transition-all cursor-pointer"
                  >
                    <Instagram size={14} />
                  </a>
                )}
                {socialMedia.tiktok && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    title="TikTok"
                    className="w-8 h-8 rounded-lg bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] hover:bg-[var(--color-accent-main)]/10 text-[var(--color-text-main)] hover:text-[#000000] flex items-center justify-center transition-all cursor-pointer"
                  >
                    <Link size={14} />
                  </a>
                )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {featuredCarsList.map((car) => (
                        <div
                          ...
                          className={`bg-gradient-to-tr ${theme.bgGradient} border border-white/10 hover:border-[#38bdf8]/30 hover:shadow-2xl hover:shadow-[#38bdf8]/10 hover:-translate-y-1.5 rounded-3xl overflow-hidden cursor-pointer shadow-lg duration-300 relative group transition-all flex flex-col justify-between`}
                        >
                          {/* Featured Ribbon */}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {filteredCars.map((car) => (
                      <div
                        key={car.id}
                        onClick={() => onSelectListing(car)}
                        className={`${theme.bgCard} border border-white/5 hover:border-[#38bdf8]/30 hover:shadow-2xl hover:shadow-[#38bdf8]/10 hover:-translate-y-1.5 rounded-3xl overflow-hidden cursor-pointer group flex flex-col justify-between transition-all duration-300`}
                      >
                        <div className="aspect-[16/10] bg-[#0F172A] relative overflow-hidden shrink-0">
                          <img
                            src={car.imageUrl}
                            alt={car.title}
                            className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"
              ...
              <div className="space-y-6">
                ...
                <div className="bg-[#1E293B] border border-white/5 p-6 rounded-3xl space-y-4 shadow-xl select-none">
                  <div className="flex justify-between items-center border-b border-white/5 pb-3">
                    ...
                    <div>
                      {hasOfficialRights ? (
                        ...
                        <span className="text-[8px] bg-sky-500/10 text-sky-400 font-mono font-bold tracking-widest uppercase px-2.5 py-1 rounded-xl border border-sky-500/20">
                          Visitor Submission
                        </span>
                      )}
                    </div>
                  ...
                  {showPublisher ? (
                    ...
                    <div className="bg-[#0F172A] p-4 py-8 rounded-2xl text-center border border-white/5 text-xs text-white/40 font-mono uppercase tracking-widest">
                      <Lock size={18} className="mx-auto mb-2 opacity-20" />
                      <p>Showroom activities are managed by verified executives</p>
                    </div>
                  )}
                </div>
          ...
          <div className="bg-[#1E293B] border border-white/10 rounded-3xl p-6 max-w-sm w-full space-y-6 text-center shadow-2xl relative">
            ...
            <div className="bg-[#0F172A] border border-white/5 rounded-2xl p-4 text-left space-y-3.5 shadow-inner">
              ...
              <div className="border-b border-white/5 pb-2">
                ...
                <a href="tel:03469085033" className="text-sm font-black text-white hover:text-slate-200 block">03469085033</a>
              </div>
              <div>
