...
export default function DetailedVehiclePostingPage({ onPostCreated, lang = 'en', currentUser, contextDealerId, dealers = [] }: DetailedVehiclePostingPageProps) {
  ...
  // STEP 4: Seller & Showroom Posting Ownership Details
  ...
  const uploadMediaFile = async (item: UploadingMedia, isVideo: boolean) => {
    const updateStatus = (id: string, updates: Partial<UploadingMedia>) => {
      if (isVideo) {
        setVideos((prev) => prev.map((m) => m.id === id ? { ...m, ...updates } : m));
      } else {
        setPhotos((prev) => prev.map((m) => m.id === id ? { ...m, ...updates } : m));
      }
    };

    updateStatus(item.id, { status: 'uploading', progress: 0 });

    try {
      const result = await uploadToCloudinary(item.file, {
        compress: !isVideo, // compress image files in uploadToCloudinary directly
        onProgress: (p) => {
          updateStatus(item.id, { progress: p });
        },
        maxRetries: 3,
        resourceType: isVideo ? 'video' : 'image',
        folder: isVideo ? 'bazar360/listings/videos' : 'bazar360/listings/photos'
      });
      ...
      console.error('[Cloudinary] Upload failed for', item.file.name, err);
      updateStatus(item.id, {
        status: 'error',
    ...
    for (const f of incoming) {
      if (f.type.startsWith('image/')) {
        ...
        try {
          ...
          console.warn('[DetailedVehiclePostingPage] Compression failed, fallback to raw:', err);
        }

        // Apply professional brand watermark
        try {
          const watermarkedDataUrl = await applyWatermark(finalFile);
          ...
          finalFile = new File([u8arr], finalFile.name, { type: mime });
        } catch (err) {
          console.warn('[DetailedVehiclePostingPage] Watermarking failed, fallback to compressed image:', err);
        }

        const previewUrl = URL.createObjectURL(finalFile);
        const newItem: UploadingMedia = {
  ...
  const handleVideoFiles = (files: FileList) => {
    ...
    for (const v of incoming) {
      if (v.type.startsWith('video/')) {
        const previewUrl = URL.createObjectURL(v);
        const newItem: UploadingMedia = {
  ...
  const removePhoto = async (idx: number) => {
    ...
    if (photo && photo.cloudinaryPublicId) {
      try {
        ...
        console.warn('[DetailedVehiclePostingPage] Error deleting photo from Cloudinary:', err);
      }
    }
  ...
  const removeVideo = async (idx: number) => {
    ...
    if (video && video.cloudinaryPublicId) {
      try {
        ...
        console.warn('[DetailedVehiclePostingPage] Error deleting video from Cloudinary:', err);
      }
    }
    setVideos((prev) => prev.filter((_, i) => i !== idx));
  };

  const uploadPdfFile = async (item: UploadingMedia) => {
    const updateStatus = (id: string, updates: Partial<UploadingMedia>) => {
      setPdfs((prev) => prev.map((m) => m.id === id ? { ...m, ...updates } : m));
    };
    ...
    try {
      ...
      console.error('[Cloudinary] PDF upload failed for', item.file.name, err);
      updateStatus(item.id, {
        status: 'error',
        error: err.message || 'Upload failed'
      });
  ...
  const handlePdfFiles = (files: FileList) => {
    if (pdfs.length >= 3) {
      alert(lang === 'ur' ? 'آپ زیادہ سے زیادہ 3 پی ڈی ایف فائلیں منتخب کر سکتے ہیں۔' : 'You can upload a maximum of 3 PDF documents.');
      return;
    }

    const incoming = Array.from(files).slice(0, 3 - pdfs.length);
    for (const p of incoming) {
      if (p.type === 'application/pdf' || p.name.toLowerCase().endsWith('.pdf')) {
        const newItem: UploadingMedia = {
        ...
        setPdfs((prev) => [...prev, newItem]);
        uploadPdfFile(newItem);
      } else {
        alert(lang === 'ur' ? 'صرف پی ڈی ایف فائلیں ہی اپ لوڈ کی جا سکتی ہیں۔' : 'Only PDF files are supported.');
      }
    }
  };

  const removePdf = async (idx: number) => {
    const pdf = pdfs[idx];
    if (pdf && pdf.cloudinaryPublicId) {
      try {
        await deleteFromCloudinary(pdf.cloudinaryPublicId, 'raw');
      } catch (err) {
        console.warn('[DetailedVehiclePostingPage] Error deleting PDF from Cloudinary:', err);
      }
    }
  ...
  const handlePublish = async () => {
    ...
    // Check for pending or failed media uploads
    const pendingUploads = [...photos, ...videos, ...pdfs].filter(m => m.status === 'uploading' || m.status === 'pending');
    if (pendingUploads.length > 0) {
      setErrorMessage(lang === 'ur' ? 'براہ کرم تمام فائلوں کے اپ لوڈ ہونے کا انتظار کریں۔' : 'Please wait for all media files to complete uploading to Cloudinary.');
      setIsSubmitting(false);
      return;
    }

    const failedUploads = [...photos, ...videos, ...pdfs].filter(m => m.status === 'error');
    if (failedUploads.length > 0) {
      setErrorMessage(lang === 'ur' ? 'کچھ فائلوں کو اپ لوڈ کرنے میں خرابی پیش آئی۔ براہ کرم ناکام فائلوں کو حذف کریں یا دوبارہ کوشش کریں۔' : 'Some media files failed to upload. Please retry uploading or delete the failed items.');
      setIsSubmitting(false);
      return;
    }
    ...
    const uploadedPhotoUrls = photos.map(p => p.cloudinaryUrl).filter(Boolean) as string[];
    const uploadedPhotoPublicIds = photos.map(p => p.cloudinaryPublicId).filter(Boolean) as string[];
    const primaryPhotoUrl = uploadedPhotoUrls[0] || 'https://images.unsplash.com/photo-1549399542-7e3f8b79c341?auto=format&fit=crop&w=600&q=80';
    const primaryPhotoPublicId = uploadedPhotoPublicIds[0] || '';

    const primaryVideo = videos.find(v => v.status === 'success');
    const primaryPdf = pdfs.find(p => p.status === 'success');
    ...
    const payload: CarListing = {
      ...
      verified: false,
      featured: false,
      dealerId: postingMode === 'showroom' ? selectedShowroomForPost : (contextDealerId || (currentUser?.uid && (currentUser?.role === 'Dealer' || currentUser?.role === 'Showroom Owner') ? currentUser.salesPodId || currentUser.uid : 'private-seller')),
      ...
      // Cloudinary specific fields saved to Firestore
      cloudinaryPublicId: primaryPhotoPublicId,
      cloudinaryPublicIds: uploadedPhotoPublicIds,
      videoUrl: primaryVideo?.cloudinaryUrl || '',
      videoCloudinaryPublicId: primaryVideo?.cloudinaryPublicId || '',
      pdfUrl: primaryPdf?.cloudinaryUrl || '',
      pdfCloudinaryPublicId: primaryPdf?.cloudinaryPublicId || '',
      pdfTitle: primaryPdf?.file.name || ''
    };
          ...
          <div className="min-h-[280px] py-2">
            <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    ...
                    >
                      ...
                      <span className="text-[10px] font-mono font-black uppercase tracking-widest">{lang === 'ur' ? 'پی ڈی ایف' : 'Upload PDF'}</span>
                    </button>
                  </div>
                  ...
                  <div className="space-y-4">
                    {photos.length > 0 && (
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
                          {photos.map((p, idx) => (
                            <div key={p.id || idx} className="aspect-square bg-slate-900 border border-white/5 rounded-xl relative overflow-hidden group">
                              {p.status === 'uploading' && (
                                <div className="absolute inset-0 bg-slate-950/80 flex flex-col items-center justify-center gap-1.5 z-10">
                                  {/* Beautiful circular progress spinner */}
                                  <div className="w-8 h-8 rounded-full border-2 border-[#38BDF8]/20 border-t-[#38BDF8] animate-spin" />
                                  <span className="text-[9px] font-mono font-bold text-[#38BDF8]">{p.progress}%</span>
                                </div>
                              )}

                              {p.status === 'error' && (
                                <div className="absolute inset-0 bg-red-950/90 flex flex-col items-center justify-center gap-1.5 z-10 p-1 text-center">
                                  <AlertCircle size={16} className="text-red-400 animate-pulse" />
                                  <button
                                    type="button"
                                    onClick={() => uploadMediaFile(p, false)}
                                    className="px-1.5 py-0.5 bg-red-500/20 text-[8px] font-mono font-black uppercase text-red-300 border border-red-500/30 rounded hover:bg-red-500/40 cursor-pointer"
                                  >
                                    Retry
                                  </button>
                                </div>
                              ...
                              <img
                                ...
                                className={`w-full h-full object-cover transition-all ${
                                  p.status === 'uploading' ? 'blur-xs saturate-50' : ''
                                }`}
                                loading="lazy"
                              />

                              {p.status === 'success' && (
                                <div className="absolute top-1 left-1 bg-emerald-500/80 text-white rounded-full p-0.5 z-10 shadow-lg">
                              ...
                              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center z-20">
                                <button 
                                  ...
                                  className="p-1.5 bg-red-500/20 text-red-400 border border-red-500/30 rounded-lg hover:bg-red-500/40 transition-colors cursor-pointer"
                                >
                                  <Trash2 size={12} />
                                </button>
                              </div>
                              <span className="absolute bottom-1 right-1 text-[7px] bg-black/70 text-zinc-400 px-1 rounded font-mono z-10">
                    ...
                    {videos.length > 0 && (
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
                          {videos.map((v, idx) => (
                            <div key={v.id || idx} className="aspect-square bg-slate-900 border border-white/5 rounded-xl relative overflow-hidden group flex items-center justify-center">
                              {v.status === 'uploading' && (
                                <div className="absolute inset-0 bg-slate-950/80 flex flex-col items-center justify-center gap-1.5 z-10">
                                  <div className="w-8 h-8 rounded-full border-2 border-emerald-500/20 border-t-emerald-500 animate-spin" />
                                  <span className="text-[9px] font-mono font-bold text-emerald-400">{v.progress}%</span>
                                </div>
                              )}
                              ...
                              {v.status === 'error' && (
                                <div className="absolute inset-0 bg-red-950/90 flex flex-col items-center justify-center gap-1.5 z-10 p-1 text-center">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => uploadMediaFile(v, true)}
                                    className="px-1.5 py-0.5 bg-red-500/20 text-[8px] font-mono font-black uppercase text-red-300 border border-red-500/30 rounded hover:bg-red-500/40 cursor-pointer"
                                  >
                                    Retry
                                  </button>
                              ...
                              <VideoIcon size={18} className={`text-emerald-400 ${v.status === 'uploading' ? 'animate-pulse text-zinc-600' : ''}`} />
                    ...
                    {pdfs.length > 0 && (
                      <div className="space-y-2">
                        <span className="text-[10px] text-zinc-500 font-mono font-black uppercase tracking-widest block">PDF Documents ({pdfs.length} / 3):</span>
                        <div className="space-y-2">
                          {pdfs.map((p, idx) => (
                            <div key={p.id || idx} className="bg-slate-900 border border-white/5 rounded-xl p-3 flex items-center justify-between gap-3 relative overflow-hidden">
                              {p.status === 'uploading' && (
                                <div className="absolute inset-0 bg-slate-950/80 flex items-center justify-between px-4 z-10">
                                  <span className="text-[10px] font-mono text-[#38BDF8]">Uploading PDF...</span>
                                  <span className="text-[10px] font-mono font-bold text-[#38BDF8]">{p.progress}%</span>
                                </div>
                              )}

                              {p.status === 'error' && (
                                <div className="absolute inset-0 bg-red-950/90 flex items-center justify-between px-4 z-10">
                                  <span className="text-[10px] font-mono text-red-400">PDF Upload Failed</span>
                                  <button
                                    type="button"
                                    onClick={() => uploadPdfFile(p)}
                                    className="px-2 py-1 bg-red-500/20 text-[9px] font-mono font-black uppercase text-red-300 border border-red-500/30 rounded cursor-pointer"
                                  >
                                    Retry
                                  </button>
                                </div>
                              ...
                              <div className="flex items-center gap-2.5">
                                <svg className="w-8 h-8 text-red-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                                  <polyline points="14 2 14 8 20 8" />
                                </svg>
                                <div className="text-left">
                                  <p className="text-xs font-bold text-white truncate max-w-[200px] sm:max-w-xs">{p.file.name}</p>
                                  <p className="text-[9px] text-zinc-400 font-mono">{(p.size / (1024 * 1024)).toFixed(2)} MB • PDF</p>
                                </div>
                              </div>
                              ...
                              <button 
                                ...
                                className="p-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 rounded-lg cursor-pointer animate-fade-in"
                              >
                                <Trash2 size={13} />
                              </button>
                            </div>
              ...
              {step === 4 && (
                ...
                >
                  {/* Admin & Showroom Posting Ownership Selector */}
                  <div className="bg-slate-900/80 border border-white/10 rounded-2xl p-4 space-y-3">
                    <label className="text-xs text-[#38BDF8] font-mono font-black uppercase block tracking-wider">
                      Posting Ownership Mode *
                    </label>
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        type="button"
                        onClick={() => setPostingMode('individual')}
                        className={`p-3 rounded-xl border text-xs font-black uppercase tracking-wider transition-all cursor-pointer flex items-center justify-center gap-2 ${
                      ...
                      <button
                        type="button"
                        onClick={() => setPostingMode('showroom')}
                        className={`p-3 rounded-xl border text-xs font-black uppercase tracking-wider transition-all cursor-pointer flex items-center justify-center gap-2 ${
                          postingMode === 'showroom'
                            ? 'bg-orange-500 text-slate-950 border-orange-500 shadow-md'
                            : 'bg-black/30 text-zinc-400 border-white/10 hover:text-white'
                        }`}
                      >
                        <Car size={14} />
                        <span>On Behalf of Showroom</span>
                      </button>
                    </div>

                    {postingMode === 'showroom' && (
                      <div className="space-y-1.5 pt-2 border-t border-white/5 animate-fade-in">
                        <label className="text-[10px] text-zinc-400 font-mono font-black uppercase block">Select Target Showroom *</label>
                        <select
                          value={selectedShowroomForPost}
                          onChange={(e) => setSelectedShowroomForPost(e.target.value)}
                          className="w-full bg-slate-950 border border-white/10 rounded-xl p-3 text-sm text-white focus:outline-none focus:border-[#38BDF8]"
                        >
                          {dealers.map((d) => (
                            <option key={d.id} value={d.id}>
                              {d.name} ({(d as any).city || d.location || 'Peshawar'})
                            </option>
                          ))}
                          {dealers.length === 0 && (
                            <option value="auto-choice-peshawar">Auto Choice Peshawar Flagship</option>
                          )}
                        </select>
                      </div>
                    )}
